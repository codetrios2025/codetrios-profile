...
const headerDetails = require("../models/headerModel");
const fs = require('fs');
...
exports.getAllservicedetailsByUrl = catchAsyncErrors(async (req, res, next) => {
 
  const { link } = req.params;

  const servicedetails = await ServiceDetails.find(req.params);
  res.status(200).json({
    success: true,
    servicedetails,
  });
...
exports.createservicedetails = catchAsyncErrors(async (req, res, next) => {
  try {
    const { titlename, link, description } = req.body;
    ...
    field.forEach((fileData, index) => {
      ...
      if (fileData) {
        ...
        console.log(`Image files for field ${index}:`, imageFiles);
    ...
    const newServiceDetail = new ServiceDetails({
      titlename,
      description,
      link,
      image,
...
exports.getservicedetailsById = catchAsyncErrors(async (req, res, next) => {
...
exports.updateservicedetails1 = catchAsyncErrors(async (req, res, next) => {
  ...
  let updatedFields = [...servicedetails.fields];
  ...
  fieldData.forEach((fileData, index) => {
    ...
    if (existingField) {
      ...
      const file = req.files[`fields[${index}][image]`];
      const newImage = file ? file[0].filename : existingField.image;
      ...
      // Handle new fields that were added
      const file = req.files[`fields[${index}][image]`];
  ...
  servicedetails.link = link || servicedetails.link;
  servicedetails.description = description || servicedetails.description;
  servicedetails.image = image || servicedetails.image;
  servicedetails.fields = updatedFields;
  console.log(servicedetails,"siddha",image)
...
exports.updateservicedetails = catchAsyncErrors(async (req, res, next) => {
  try {
...
exports.deleteservicedetails = catchAsyncErrors(async (req, res, next) => {
  const servicedetails = await ServiceDetails.findById(req.params.id);
  if (!servicedetails) {
    return next(new ErrorHander('ServiceDetails not found', 404));
  }
...
exports.deleteImageFromTeam = catchAsyncErrors(async (req, res, next) => {
  ...
  const servicedetails = await ServiceDetails.findById(serviceId);
  if (!servicedetails) {
    return next(new ErrorHander('ServiceDetails not found', 404));
  }
...
exports.removeFieldFromServiceDetails = catchAsyncErrors(async (req, res, next) => {
  const { serviceId, fieldId } = req.params;

  try {
    // Find the service by ID and update it by removing the specified field
    const updatedService = await ServiceDetails.findByIdAndUpdate(
      serviceId,
