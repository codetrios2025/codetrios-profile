...
const DownloadType = require("../models/downloadTypeModel");


// // @desc    Get all DownloadType Links

exports.getAllDownloadType = catchAsyncErrors(async (req, res, next) => {
    const DownloadTypes = await DownloadType.find();
    res.status(200).json({
      success: true,
      DownloadTypes,
    });
...
exports.createDownloadType= catchAsyncErrors(async (req,res,next) =>{
    ...
    const newDownloadType = new DownloadType({ title,order ,pageType});
...
exports.getDownloadTypeById = catchAsyncErrors(async (req,res,next)=>{
  const DownloadTypes = await DownloadType.findById(req.params.id);
  if (!DownloadTypes) {
      return next(
...
exports.getAllDownloadTypebyTitle = catchAsyncErrors(async (req, res, next) => {
  ...
  const DownloadTypes= await DownloadType.find(req.query);
  res.status(200).json({
      success: true,
      DownloadTypes,
  });
