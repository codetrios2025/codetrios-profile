...
const FoodDownloads = require("../models/fooddownloadModel");

// @desc    Get all Downloads
exports.getAllDownload = catchAsyncErrors(async (req, res, next) => {
    const downloads= await FoodDownloads.find();
    res.status(200).json({
        success: true,
        downloads,
    });
...
exports.getAllDownloadbyTitle = catchAsyncErrors(async (req, res, next) => {
  ...
  const downloads= await FoodDownloads.find(req.query);
  res.status(200).json({
      success: true,
      downloads,
  });
...
exports.createDownload = catchAsyncErrors(async (req, res, next) => {
    ...
    pdfs.forEach((pdfData, index) => {
      if (pdfData.language) {
        ...
        const newdownload = new FoodDownloads({
          downloadtype,
            title,
            order,
            pageType,
...
exports.getDownloadById = catchAsyncErrors(async (req, res, next) => {
    const download = await FoodDownloads.findById(req.params.id);
    if (!download) {
