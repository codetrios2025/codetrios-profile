...
const ProjectServiceSection = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await getItem('projectservices');
      setItems(response.data.projectservice);
    } catch (error) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editing) {
        const response = await axios.put(`${constants.API_BASE_URL}projectservices/${currentItem._id}`, formData);
        ...
        const response = await axios.post(`${constants.API_BASE_URL}projectservices`, formData);
  ...
  const handleDelete = async (data) => {
    try {
      await axios.delete(`${constants.API_BASE_URL}projectservices/${data._id}`);
  ...
  const handleImageDelete = async (data,fileName) => {
    ...
    try {
      await axios.delete(`${constants.API_BASE_URL}projectservices/teamimage/${data._id}`, {
                ...
                <tr key={idx}>
                  ...
                  <td>
                    {item.image.map((img, i) => (
                      ...
                      <Button variant="danger" onClick={() => handleImageDelete(item,img.fileName)}><FaTrash /></Button></>
                    ))}
                  </td>
                  <td>
...
export default ProjectServiceSection;
